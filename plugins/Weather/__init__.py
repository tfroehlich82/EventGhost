...
eg.RegisterPlugin(
    ...
    icon = ("iVBORw0KGgoAAAANSUhEUgAAACgAAAAoCAMAAAC7IEhfAAAAwFBMVEXm5uerz+ZocXj9"
...
class GetWeatherFromGoogle(eg.ActionClass):
    description = ("Fetches weather report from Google \n"
	           "PARAMETERS: \n"
...
class GetCountriesFromGoogle(eg.ActionClass):
    description = ("Get list of countries in specified language from Google \n"
                   "PARAMETERS: \n"
      		   "* hl: the language parameter (language code). Default value is empty string, in this case Google will use English. \n"
    		   "RETURNS: \n"
...
class GetCitiesFromGoogle(eg.ActionClass):
    description = ("Get list of cities of necessary country in specified language from Google \n"
                   "PARAMETERS: \n"
...
class GetWeatherFieldValue(eg.ActionClass):
    description = ("Get a particular parameter value from the earlier retrieved Weather Information from Google \n"
                   "PARAMETERS: \n"
...
class LogResult(eg.ActionClass):
    description = ("Shows the returned information from the previous Action in the EventGhost Log\n"
    		  )
